...
void TextureAtlas::build() {
  for (int y = 0; y < TileSize; ++y) {
    for (int x = 0; x < TileSize; ++x) {
      ...
      set(TextureId::Concrete, x, y, seam ? rgb(43, 50, 54) : rgb(78 + grit, 88 + grit, 91 + grit));

      const bool crack = (x > 8 && x < 11 && y > 5 && y < 20) || (y == 19 && x > 9 && x < 23) ||
                         (x == 22 && y > 18 && y < 28);
      set(TextureId::DamagedConcrete, x, y, crack ? rgb(27, 29, 31) : rgb(68 + grit, 73 + grit, 72 + grit));

      const bool metalSeam = x % 16 == 0 || y % 16 == 0;
      const bool rivet = (x % 16 == 3 || x % 16 == 13) && (y % 16 == 3 || y % 16 == 13);
      set(TextureId::IndustrialMetal, x, y,
          rivet ? rgb(142, 151, 150) : metalSeam ? rgb(35, 45, 50) : rgb(72 + grit / 2, 83 + grit / 2, 87 + grit / 2));

      const bool rust = noise(x / 2, y / 2, 8) > 20;
      set(TextureId::RustedMetal, x, y, rust ? rgb(112 + grit, 55 + grit / 2, 31) : rgb(65 + grit / 2, 69 + grit / 2, 66 + grit / 2));

      const bool doorEdge = x < 3 || x > 28 || x % 8 == 0;
      const bool doorLight = y > 5 && y < 9 && x > 5 && x < 27;
      set(TextureId::SecurityDoor, x, y, doorLight ? rgb(45, 225, 211) : doorEdge ? rgb(35, 42, 44) : rgb(115 + grit / 3, 93 + grit / 3, 48));
      ...
      set(TextureId::ComputerPanel, x, y, screen ? (scan ? rgb(32, 116, 109) : rgb(38, 207, 177)) : rgb(34, 42, 49));

      const bool plateSeam = x % 16 == 0 || y % 16 == 0;
      set(TextureId::FloorPlate, x, y, plateSeam ? rgb(25, 29, 32) : rgb(57 + grit / 3, 62 + grit / 3, 63 + grit / 3));
      const bool stain = noise(x / 3, y / 3, 14) > 23;
      set(TextureId::DirtyFloor, x, y, stain ? rgb(48, 43, 34) : rgb(67 + grit / 3, 65 + grit / 3, 57 + grit / 3));
      set(TextureId::HazardFloor, x, y, stripe ? rgb(151, 105, 25) : rgb(34, 36, 36));

      const bool ceilingSeam = x % 16 == 0 || y % 16 == 0;
      set(TextureId::CeilingPanel, x, y, ceilingSeam ? rgb(30, 37, 42) : rgb(52 + grit / 4, 62 + grit / 4, 68 + grit / 4));
      const bool pipe = (x > 5 && x < 10) || (x > 21 && x < 26);
      const bool pipeEdge = x == 6 || x == 9 || x == 22 || x == 25;
      set(TextureId::CeilingPipes, x, y, pipeEdge ? rgb(27, 31, 34) : pipe ? rgb(93, 72, 54) : rgb(39 + grit / 4, 44 + grit / 4, 47 + grit / 4));
    }
  }
}
