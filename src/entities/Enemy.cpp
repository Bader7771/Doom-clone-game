...
void Enemy::damage(int n,Vec2 direction,float force){if(state==EnemyState::Dead)return;health-=n;pos+=direction*(force*.18f);painFlash=.16f;if(health<=0)state=EnemyState::Dead;else state=EnemyState::Chase;}
void Enemy::update(float dt,Player& p,const Level& l){
  anim+=dt;painFlash=std::max(0.f,painFlash-dt);attackCooldown=std::max(0.f,attackCooldown-dt);if(state==EnemyState::Dead)return;
