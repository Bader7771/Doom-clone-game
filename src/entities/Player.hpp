...
public:
  ...
  bool hasKey{false};
  float hurtFlash{};
  float movementAmount() const { return movementAmount_; }
  bool running() const { return running_; }
  float viewKick() const { return viewKick_; }
  float screenShake() const { return screenShake_; }
  float lateralMovement() const { return lateralMovement_; }
  std::uint32_t damageSerial() const { return damageSerial_; }
  int lastDamage() const { return lastDamage_; }
  float damageSide() const { return damageSide_; }
private:
