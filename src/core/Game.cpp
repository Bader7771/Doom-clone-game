...
int Game::run(){Uint64 last=SDL_GetTicksNS();bool captureShot=std::getenv("VOIDLOCK_CAPTURE_FIRE")!=nullptr;while(input_.update()){if(input_.fullscreenToggle()){fullscreen_=!fullscreen_;SDL_SetWindowFullscreen(window_,fullscreen_);}Uint64 now=SDL_GetTicksNS();float dt=std::min(.05f,(now-last)/1000000000.f);last=now;if(captureShot){shotgun_.update(0,true,player_,level_,enemies_,audio_);captureShot=false;}update(dt);renderer_.draw(level_,player_,enemies_,shotgun_,won_);if(player_.health<=0){player_.health=100;player_.pos={2.5f,2.5f};}}return 0;}
